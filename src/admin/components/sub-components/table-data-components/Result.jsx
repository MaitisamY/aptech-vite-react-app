...
import { tests, students } from "../../Datatable-data";
...
export default function ResultData(props) {
    ...
    const filteredResults = results.filter(result => result.date.toLowerCase().includes(searchQuery.toLowerCase()));
                    ...
                    <tbody>
                        {resultsSliced.map((resultsBar) => (
                            <tr key={resultsBar.id}>
                                <td>
                                    {students.filter(bar => bar.id === resultsBar.student).map((bar) => (
                                        <p key={bar.id}>{bar.name}</p>))}
                                </td>
                                <td>
                                    {tests.filter(bar => bar.id === resultsBar.test).map((bar) => (
                                        <p key={bar.id}>{bar.title}</p>))}
                                </td>
                                <td>{resultsBar.score}</td>
                                <td>{resultsBar.date}</td>
                                <td>
